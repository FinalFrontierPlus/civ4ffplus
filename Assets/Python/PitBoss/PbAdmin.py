...
def monthNametoInt( name ):
	...
	if name == "February": return "02"
	if name == "March": return "03"
	if name == "April": return "04"
	if name == "May": return "05"
	if name == "June": return "06"
	if name == "July": return "07"
	if name == "August": return "08"
	if name == "September": return "09"
...
class AdminFrame(wx.Frame):
	def __init__(self, parent, ID, title):
		...
		#DLP
		self.title = wx.StaticText(self, -1, PB.getGamename() + " - " + PB.getGamedate(False)[-5:][:-3] + monthNametoInt(PB.getGamedate(False)[:-9]) + " AD")
	...
	def update(self):
		...
		if (self.gameTurn != PB.getGameturn()):
			#DLP
			self.title.SetLabel(PB.getGamename() + " - " + PB.getGamedate(False)[-5:][:-3] + monthNametoInt(PB.getGamedate(False)[:-9]) + " AD")
			self.gameTurn = PB.getGameturn()
