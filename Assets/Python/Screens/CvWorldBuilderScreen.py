...
from CvSolarSystem import *		#Added in Final Frontier Worldbuilder: TC01
import CvAI						#Added in Final Frontier Worldbuilder: TC01
import FeatureRemoval			#Temp: TC01

gc = CyGlobalContext()
ArtFileMgr = CyArtFileMgr()
localText = CyTranslator()
AI = CvAI.CvAI()				#Added in Final Frontier Worldbuilder: TC01
...
#Changed in Final Frontier Worldbuilder: TC01
				iBuildingsPerSystem = self.m_pCurrentPlot.getPlotCity().getNumRealBuilding(iBuildingType-1)
#				FinalFrontier = CvEventInterface.getEventManager() #FFPBUG
				...
				if not pBuildingPlanet.isHasBuilding(iBuildingType-1):
					pBuildingPlanet.setHasBuilding(iBuildingType-1, true)
					self.m_pCurrentPlot.getPlotCity().setNumRealBuilding(iBuildingType-1, iBuildingsPerSystem+1)
...
#								FinalFrontier = CvEventInterface.getEventManager().FinalFrontier #FFPBUG
								pSystem = getSystemAt(self.m_pCurrentPlot.getX(), self.m_pCurrentPlot.getY()) #FFPBUG
#FFPBUG								FinalFrontier.apSystems.remove(pSystem)
#FFPBUG								FinalFrontier.iNumSystems -= 1
...
#								FinalFrontier = CvEventInterface.getEventManager()
							...
							if i == gc.getInfoTypeForString('FEATURE_OASIS'):
								for xLoop in range((iX - 2), (iX + 3)):
									for yLoop in range((iY - 2), (iY + 3)):
										if ((xLoop == iX - 2) or (xLoop == iX + 2)) and ((yLoop == iY - 2) or (yLoop == iY + 2)):
											...
											pEffectPlot.setFeatureType(gc.getInfoTypeForString('FEATURE_GRAV_FIELD'), 0)
							if i == gc.getInfoTypeForString('FEATURE_JUNGLE'):
								for xLoop in range((iX - 2), (iX + 3)):
									for yLoop in range((iY - 2), (iY + 3)):
										if ((xLoop == iX - 2) or (xLoop == iX + 2)) and ((yLoop == iY - 2) or (yLoop == iY + 2)):
											continue
											...
											pEffectPlot = CyMap().plot(xLoop, yLoop)
											pEffectPlot.setFeatureType(gc.getInfoTypeForString('FEATURE_SUPERNOVA_AREA'), 0)
#End of Final Frontier Worldbuilder
...
#Added in Final Frontier Worldbuilder: TC01
					iBuildingsPerSystem = self.m_pCurrentPlot.getPlotCity().getNumRealBuilding(iBuildingType-1)
#					FinalFrontier = CvEventInterface.getEventManager()
					pSystem = getSystemAt(self.m_pCurrentPlot.getX(), self.m_pCurrentPlot.getY())
					pBuildingPlanet = pSystem.getPlanet(pSystem.getBuildingPlanetRing())
					if pBuildingPlanet.isHasBuilding(iBuildingType-1):
						pBuildingPlanet.setHasBuilding(iBuildingType-1, false)
					self.m_pCurrentPlot.getPlotCity().setNumRealBuilding(iBuildingType-1, iBuildingsPerSystem-1)
#End of Final Frontier Worldbuilder
		elif ((self.m_bNormalMap) and (self.m_normalMapTabCtrl.getActiveTab() == self.m_iImprovementTabID)):
...
#Function altered in Final Frontier Worldbuilder: TC01
	def setEditButtonClicked(self, argsList):
		iIndex = int(argsList)
#		FinalFrontier = CvEventInterface.getEventManager()	
		pSystem = getSystemAt(self.m_pActivePlot.getX(), self.m_pActivePlot.getY()) #FFPBUG
		iNumNewBuildings = 0
		if (self.m_bUnitEditCtrl):
			...
			if (self.m_pActivePlot.getPlotCity().getNumBuilding(iIndex) > 0):
				...
				for iPlanet in range(pSystem.getNumPlanets()):
					pPlanet = pSystem.getPlanetByIndex(iPlanet)
					pPlanet.setHasBuilding(iIndex, false)
			else:
				iNum = self.m_pActivePlot.getPlotCity().getNumBuilding(iIndex)
				for iPlanet in range(pSystem.getNumPlanets()):
					pPlanet = pSystem.getPlanetByIndex(iPlanet)
					if not pPlanet.isHasBuilding(iIndex):
						pPlanet.setHasBuilding(iIndex, true)
						iNum += 1
