...
def getDescription():
	return "This is the FinalFrontierFlat script, but it places a pair of Wormholes some distance away from each other, and on Standard, Large, and Huge maps, an additional pair."
...
#Wormhole features -- TC01
		self.featureWormhole = self.gc.getInfoTypeForString("FEATURE_WORMHOLE")
		self.featureRedWormhole = self.gc.getInfoTypeForString("FEATURE_RED_WORMHOLE")
		self.featurePurpleWormhole = self.gc.getInfoTypeForString("FEATURE_PURPLE_WORMHOLE")
#End of wormholes
...
#Added in Wormholes to check for wormhole room: TC01
	def checkWormholeRoom(self, pPlot, iWormholeType):
		iX = pPlot.getX()
		iY = pPlot.getY()
		for iHoleX in range((iX - 2), (iX + 3)):
			for iHoleY in range((iY - 2), (iY + 3)):
		...
		if self.map.getWorldSize() == self.gc.getInfoTypeForString("WORLDSIZE_HUGE"):
			for iHoleX in range((iX - 32), (iX + 33)):
				for iHoleY in range((iY - 20), (iY + 21)):
					if ((iHoleX == iX - 2) or (iHoleX == iX + 2)) and ((iHoleY == iY - 2) or (iHoleY == iY + 2)):
						continue #Skip the four corners.
					else:
						pPlot = self.map.sPlot(iHoleX, iHoleY)
						if pPlot.getFeatureType() == iWormholeType:
							return False
		if self.map.getWorldSize() == self.gc.getInfoTypeForString("WORLDSIZE_LARGE"):
			for iHoleX in range((iX - 26), (iX + 27)):
				for iHoleY in range((iY - 16), (iY + 17)):
					if ((iHoleX == iX - 2) or (iHoleX == iX + 2)) and ((iHoleY == iY - 2) or (iHoleY == iY + 2)):
						continue #Skip the four corners.
					else:
						pPlot = self.map.sPlot(iHoleX, iHoleY)
						if pPlot.getFeatureType() == iWormholeType:
							return False
		if self.map.getWorldSize() == self.gc.getInfoTypeForString("WORLDSIZE_STANDARD"):
			for iHoleX in range((iX - 21), (iX + 22)):
				for iHoleY in range((iY - 13), (iY + 14)):
					if ((iHoleX == iX - 2) or (iHoleX == iX + 2)) and ((iHoleY == iY - 2) or (iHoleY == iY + 2)):
						continue #Skip the four corners.
					else:
						pPlot = self.map.sPlot(iHoleX, iHoleY)
						if pPlot.getFeatureType() == iWormholeType:
							return False
		if self.map.getWorldSize() == self.gc.getInfoTypeForString("WORLDSIZE_SMALL"):
			for iHoleX in range((iX - 16), (iX + 17)):
				for iHoleY in range((iY - 10), (iY + 11)):
					if ((iHoleX == iX - 2) or (iHoleX == iX + 2)) and ((iHoleY == iY - 2) or (iHoleY == iY + 2)):
						continue #Skip the four corners.
					else:
						pPlot = self.map.sPlot(iHoleX, iHoleY)
						if pPlot.getFeatureType() == iWormholeType:
							return False
		if self.map.getWorldSize() == self.gc.getInfoTypeForString("WORLDSIZE_TINY"):
			for iHoleX in range((iX - 7), (iX + 8)):
				for iHoleY in range((iY - 5), (iY + 6)):
					if ((iHoleX == iX - 2) or (iHoleX == iX + 2)) and ((iHoleY == iY - 2) or (iHoleY == iY + 2)):
						continue #Skip the four corners.
					else:
						pPlot = self.map.sPlot(iHoleX, iHoleY)
