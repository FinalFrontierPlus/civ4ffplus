...
class CvPediaUnit(CvPediaScreen.CvPediaScreen):
	...
	def placeLinks(self, bRedraw):
		...
		for unit in unitList:
			if (not gc.getUnitInfo(unit[1]).isGraphicalOnly()):
				if gc.getUnitInfo(unit[1]).isGoldenAge() or gc.getUnitInfo(unit[1]).getUnitAIType(int(UnitAITypes.UNITAI_GENERAL)):
					continue
				if (not gc.getDefineINT("CIVILOPEDIA_SHOW_ACTIVE_CIVS_ONLY") or not gc.getGame().isFinalInitialized() or gc.getGame().isUnitEverActive(unitsList[iI][1])):
					if bRedraw:
						screen.appendListBoxStringNoUpdate( self.top.LIST_ID, unit[0], WidgetTypes.WIDGET_PEDIA_JUMP_TO_UNIT, unit[1], 0, CvUtil.FONT_LEFT_JUSTIFY )
					if unit[1] == self.iUnit:
						iSelected = i
					i += 1
