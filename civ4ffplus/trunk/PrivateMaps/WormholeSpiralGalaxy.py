##############################################################################
## File: SpiralGalaxy.py version 1.0
...
class FeaturePlacer :
    ...
    def initFeatureDefList(self):
        ...
        ft = FeatureDef(self.featureRedWormhole)
        ft.arcLength = 0.5
        ft.arcLengthRange = 0.7
        ft.arcLengthMaxChance = 1.0
        ft.armDistance = 0.2
        ...
        ft = FeatureDef(self.featurePurpleWormhole)
        ft.arcLength = 0.5
        ft.arcLengthRange = 0.7
        ft.arcLengthMaxChance = 1.0
        ft.armDistance = 0.2
    ...
    def generateFeatures(self):
        ...
        self.featureRedWormhole = gc.getInfoTypeForString("FEATURE_RED_WORMHOLE")
        self.featurePurpleWormhole = gc.getInfoTypeForString("FEATURE_PURPLE_WORMHOLE")
        self.initFeatureDefList()

        game = CyGame()

        #Define wormhole order based on map option
        iWormholeOrder = mmap.getCustomMapOption(1)
        if iWormholeOrder == 0:
...
#Wormholes and Red Wormholes placing code
        if not mmap.getWorldSize() == gc.getInfoTypeForString("WORLDSIZE_DUEL"):
			numPlaced = 0
			iterations = 0
			while(True):
				iterations += 1
				if iterations > 2000:
					...
					break
				if numPlaced == numWormholes:
					break
				for n in range(len(plotList)):
					x,y = plotList[n]
					if self.shouldPlaceFeature(x,y,iTinyWormhole) and self.checkForRoom(x,y,iTinyWormhole) and self.checkForWormholeRoom(x,y,iTinyWormhole):
						numPlaced += 1
						self.placeWormhole(x,y,iTinyWormhole)
						if numPlaced == numWormholes:
							break
			print "Placed %(p)d out of %(n)d wormholes" % {"p":numPlaced,"n":numWormholes}

        if (mmap.getWorldSize() == gc.getInfoTypeForString("WORLDSIZE_HUGE") or mmap.getWorldSize() == gc.getInfoTypeForString("WORLDSIZE_LARGE") or mmap.getWorldSize() == gc.getInfoTypeForString("WORLDSIZE_STANDARD")):
			numPlaced = 0
			iterations = 0
			while(True):
				iterations += 1
				...
				for n in range(len(plotList)):
					x,y = plotList[n]
					if self.shouldPlaceFeature(x,y,iStandardWormhole) and self.checkForRoom(x,y,iStandardWormhole) and self.checkForWormholeRoom(x,y,iStandardWormhole):
						numPlaced += 1
						self.placeWormhole(x,y,iStandardWormhole)
						if numPlaced == numWormholes:
							break
			print "Placed %(p)d out of %(n)d red wormholes" % {"p":numPlaced,"n":numWormholes}
        if mmap.getWorldSize() == gc.getInfoTypeForString("WORLDSIZE_HUGE"):
			numPlaced = 0
			iterations = 0
			while(True):
				iterations += 1
				...
				for n in range(len(plotList)):
					x,y = plotList[n]
					if self.shouldPlaceFeature(x,y,iHugeWormhole) and self.checkForRoom(x,y,iHugeWormhole) and self.checkForWormholeRoom(x,y,iHugeWormhole):
						numPlaced += 1
						self.placeWormhole(x,y,iHugeWormhole)
						if numPlaced == numWormholes:
							break
			print "Placed %(p)d out of %(n)d purple wormholes" % {"p":numPlaced,"n":numWormholes}
#End of red wormhole/wormhole placing
        ...
        # Now remove all the placeholder Flood Plains. - origional FF code
#	for iPlotLoop in range(CyMap().numPlots()):
#		pPlot = CyMap().plotByIndex(iPlotLoop)
...
def getDescription():
	...
	"""
	return "A Spiral Galaxy with two wormholes connecting two random points on the map."
