...
/*********************************************************************/
/** SPEEDTWEAK **/
/** Pathfinding can be really slow. I have seen this function taking up to a few seconds sometimes. And in Mods that greatly change the game it can get a lot worse.
/** reason unknown. most players don't understand how trade is calculated so making it random does not seem to cause much issues 
/** TC01 Change: Use a "Calculate Trade Route Paths" gameoption rather than the random thing to give players an option*/
						if (GC.getGame().isOption(GAMEOPTION_CALC_TRADE_ROUTES))
						{
						...
						{
							iValue = GC.getGame().getSorenRandNum(10000, "Trade Route");
							if (iValue < iBestValue)
							{
								iBestValue = iValue;
								pBestCity = pLoopCity;
